...
void canTest1Callback(CAN_message_t msg_in) {
  // for (int i = 0; i < 3; i++) {
  //   est.x[i] = 0;
  // }
  Serial.println("canTest1Callback");
...
void canbus_loop(void) {
  ...
  if (fc.read(msg_in)) {
    // respond to message
    Serial.print("CAN test 2 received message with decoded id: ");
